local function parse_package_info()
	-- Find workspace root (where packageInfo is located)
	...
	for line in content:gmatch("[^\r\n]+") do
		...
		elseif in_packages_block then
			-- Match package entries like "CallioExperts-1.0 = .;"
		...
		actions = {
			...
			["ctrl-s"] = function(selected)
				if selected and #selected > 0 then
					...
					-- Open in a horizontal split with oil
					vim.cmd("split")
					require("oil").open(package_path)
				end
			end,
			["ctrl-v"] = function(selected)
				if selected and #selected > 0 then
					...
					-- Open in a vertical split with oil
					vim.cmd("vsplit")
					require("oil").open(package_path)
				end
			end,
